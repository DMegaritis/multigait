...
class TestKimSL:
    ...
    def test_invalid_version_parameter(self, version):
        ...
        with pytest.raises(ValueError):
            KimSL(version="invalid_version").calculate(
                data=pd.DataFrame(
    ...
    def test_random_noise_effect(self, noise_std):
        ...
        sl_clean = KimSL(version="wrist").calculate(
        ...
        sl_noisy = KimSL(version="wrist").calculate(
        ...
        assert diff < 0.1
        # Check stride length consistency
        np.testing.assert_allclose(
            sl_noisy.stride_length_per_sec_["stride_length_m"].values,
            sl_noisy.step_length_per_sec_["step_length_m"].values * 2,
            rtol=1e-5,
        )
