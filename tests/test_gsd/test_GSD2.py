...
import pandas as pd
import pytest
from multigait.GSD import HickeyGSD  # Adjust import path
...
class TestHickeyGSD:
    ...
    def test_empty_input(self, version, cwb):
        ...
        gs_list = (
            HickeyGSD(version=version, cwb=cwb)
            .detect(data, sampling_rate_hz=100)
            .gs_list_
        )
    ...
    def test_real_wrist_data(self, version, cwb):
        ...
        gs_list = (
            HickeyGSD(version=version, cwb=cwb)
            .detect(imu_data, sampling_rate_hz=100)
            .gs_list_
        )
